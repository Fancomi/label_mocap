...
ModelOutput = namedtuple(
    "ModelOutput", ["vertices", "joints", "joints_from_verts", "rot_mats", "feet", "joints_wham"]
)
...
class SMPL_layer(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "J_regressor_feet", to_tensor(to_np(feet_jregressor), dtype=dtype)
        )
    ...
    def forward(
        ...
        # 统一行为: 先减root_joint(pelvis中心化), 再加transl
        root_j = joints[:, self.root_idx_smpl, :].unsqueeze(1).detach()
        ...
        output = ModelOutput(
            ...
            joints=joints,
            rot_mats=rot_mats,
            feet=self.J_regressor_feet@vertices,
